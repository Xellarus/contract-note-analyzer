...
interface ImportResult { added: number; addedNames: string[]; pricesUpdated: number; pricesTotal: number; }
  ...
  const runImport = async () => {
    ...
    try {
      ...
      const sec = await appendScreenerSecurities(SCRIP_MASTER_SPREADSHEET_ID, master,
        securities.map(s => ({ isin: s.isin, name: s.name, bse: s.bse, nse: s.nse })));
      ...
      setResult({ added: sec.added, addedNames: sec.addedNames, pricesUpdated: pr.updated, pricesTotal: pr.total });
    } catch (e: any) {
    ...
    <div className="max-w-4xl mx-auto animate-fadeIn space-y-5">
      <div>
        ...
        <p className="text-xs text-slate-500 mt-0.5">
          ...
          to the scrip master and refresh current prices used to value your holdings.
        </p>
      </div>

// Bundles the shared-reconciler test with esbuild, stubbing the two Vite-only imports that
...
const out = `${process.env.TEMP || ROOT}/.recon-bundle.mjs`;
await esbuild.build({
  entryPoints: [`${ROOT}/tmp-recon.ts`],
  bundle: true,
  platform: 'node',
  format: 'esm',
  target: 'node20',
